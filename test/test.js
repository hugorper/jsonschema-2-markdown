...
test('Test readme-sample.json with template.', function(t) {
	const file = "readme-sample";

	var jsonObject = require('./json-schema-template/' + file + '.json');
	
	// comment to generate markdown files 
	var markdown = fs.readFileSync('./test/json-schema-markdown/' + file + '.md', 'utf8');
...
`;
	...
	// uncomment to generate markdown files
	/*
	fs.appendFile('./test/json-schema-markdown/' + file + '.md', mdOutput, 'utf8', function (err) {
		if (err) throw err;
		console.log('Saved!');
	}); 
	*/
	...
	validator(jsonObject); // assert that JSON schema files are valid
	// comment to generate markdown files 
	t.equal(mdOutput, markdown, 'markdown file "' + file + '" should match parser output');
	// uncomment to generate markdown files 
	//t.equal("1", "1", "ok");
